---
name: goal-mode
description: Handler for Codex goal mode. Use this skill whenever the user message starts with `/goal`, contains `/goal`, asks to enter goal mode, or contains a `goal_context` block generated by Codex from a `/goal` command. First turn only initializes goal-N files and goal-current, persists a zh-CN/en-US language policy, outputs exactly GOAL_INIT_DONE, then later runs one task per session, verifies evidence, commits code changes in git repos, stops after each task boundary, and continues until complete.
---
...
- Persist and follow a `zh-CN` or `en-US` language policy for user-facing progress, task records, and final reports.
- Guard against pre-existing worktree changes before task, checkpoint, or final-review work.
- Split explicitly named deliverables into separate independently verifiable tasks during initialization.
...
- If repository commit hooks reject the literal goal-mode subject, keep the goal-mode boundary marker in the commit body and `tasks.md`.
...
- you are about to run a checkpoint or final review in the same session that completed a task boundary
- you are about to merge multiple explicitly named deliverables into one task instead of splitting them
...
- you are about to ignore the persisted `Language policy`
...
- a repository commit hook rejected the literal goal-mode subject and you are about to commit without a `Goal-mode boundary:` marker in the body
