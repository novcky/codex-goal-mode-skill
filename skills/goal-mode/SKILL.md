---
name: goal-mode
description: Handler for Codex goal mode. Use this skill whenever the user message starts with `/goal`, contains `/goal`, asks to enter goal mode, or contains a `goal_context` block generated by Codex from a `/goal` command. Initializes goal-N files and a goal-current pointer before edits, runs one task per session, stops on red flags, verifies evidence, updates tasks.md, and continues until complete.
---
